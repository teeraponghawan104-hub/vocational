...
export default function Part1({ initialAnswers = [], onChange, onComplete }: Props) {
  ...
  const isComplete = part1Questions.every(q => answers[q.id] !== undefined && answers[q.id] !== null);

  return (
    <div className="space-y-8 animate-in fade-in duration-500 flex flex-col h-full">
      <div className="mb-4 shrink-0">
        <span className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1 block">ส่วนที่ 1 / 3</span>
        <h2 className="text-2xl font-bold text-slate-800 leading-tight">แบบทดสอบความสนใจด้านอาชีพ (Vocational Interest Test)</h2>
        <p className="text-slate-500 mt-2">โปรดพิจารณาข้อความต่อไปนี้และเลือกคำตอบที่ตรงกับความสนใจของคุณมากที่สุด</p>
      </div>
      ...
      <div className="pt-6 border-t border-slate-200 shrink-0">
        <button
          disabled={!isComplete}
          onClick={() => {
            const formatted = part1Questions.map(q => ({
              questionId: q.id,
              choice: answers[q.id]
            }));
        ...
        >
          {isComplete ? "ถัดไป (ส่วนที่ 2)" : `กรุณาตอบให้ครบทุกข้อ (${Object.values(answers).filter(v => v !== null).length} / 54)`}
